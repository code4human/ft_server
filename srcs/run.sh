...
# create table for wordpress
service mysql start
echo "CREATE DATABASE IF NOT EXISTS wordpress;" \
	| mysql -u root --skip-password
echo "CREATE USER IF NOT EXISTS 'taeekim'@'localhost' IDENTIFIED BY 'taeekim_password';" \
	| mysql -u root --skip-password
echo "GRANT ALL PRIVILEGES ON wordpress.* TO 'taeekim'@'localhost' WITH GRANT OPTION;" \
	| mysql -u root --skip-password
